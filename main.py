...
def rose_board_to_gui_board(rose_board):
    mapper = {' ': None, 'x':'x', '0':'o'}
    rose_board = [mapper[rose_tolken] for rose_tolken in rose_board]
    return [rose_board[:3],rose_board[3:6],rose_board[6:9]]

def gui_board_to_rose_board(gui_board):
    mapper = {None:' ', 'x':'x', 'o':'0'}
    flattened = []
...
def game_setup():
    ...
    txtsurf = font.render("Player1: Press 0 to play as human, or 1 to play as RANDOM_AGENT, or 2 to play as AI_AGENT", True, white)
    screen.blit(txtsurf,(200 - txtsurf.get_width() // 2, 150 - txtsurf.get_height() // 2))
    if pg.event.get()[0].type == pg.KEYDOWN:
        if pg.event.get()[0].key == pg.K_0:
            player1_agent = 0
        elif pg.event.get()[0].key == pg.K_1:
            player1_agent = 1
        else:
            player1_agent = 2
    
    txtsurf = font.render("Player2: Press 0 to play as human, or 1 to play as RANDOM_AGENT, or 2 to play as AI_AGENT", True, white)
    screen.blit(txtsurf,(200 - txtsurf.get_width() // 2, 150 - txtsurf.get_height() // 2))
    if pg.event.get()[0].type == pg.KEYDOWN:
        if pg.event.get()[0].key == pg.K_0:
            player2_agent = 0
